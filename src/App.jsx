  ...
  return (
    <div>
      <div className="topbar">
        ...
        {isAdmin && <button className="btn btn-primary" onClick={()=>setModal("membre")}>+ Ajouter</button>}
      </div>
      ...
      {isMobile ? (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {fil.map(m=>(
            <div key={m.id} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:16,padding:16,boxShadow:"var(--shadow)"}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                <div className="sm-av" style={{background:m.c||"#4f6ef7",width:40,height:40,fontSize:16}}>{(m.nom||"?")[0]}</div>
                <div style={{flex:1}}>
              ...
              <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                <span className="pill pill-blue">{m.club}</span>
                <span className="pill pill-green">{m.role}</span>
              </div>
              {isAdmin && (
                <div style={{display:"flex",gap:8}}>
                  <button className="btn btn-primary btn-sm" style={{flex:1}} onClick={()=>{setMembreEdit(m);setForm({prenom:m.nom,email:m.email,club:m.club,role:m.role});setModal("membre");}}>✏️ Modifier</button>
                  <button className="btn btn-red btn-sm" style={{flex:1}} onClick={()=>{deleteDoc(doc(db,"membres",m.id));notify("Membre supprimé.","🗑️");}}>🗑️ Supprimer</button>
                </div>
              )}
            </div>
          ))}
